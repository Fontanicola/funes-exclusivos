...
function PaymentForm({ liquidacion }: { liquidacion: Liquidacion }) {
  ...
  return (
    <form action={formAction as any} className="space-y-3 rounded-md border border-[#E5E7EB] bg-[#FAFAFA] p-4">
      ...
      <div className="space-y-2">
        ...
        <select id="medio_caja" name="medio_caja" defaultValue="efectivo" required className="h-10 w-full rounded-md border border-[#E5E7EB] bg-white px-3 text-sm text-[#111827] outline-none transition focus:border-[#8A1538] focus:ring-2 focus:ring-[#E9B8C6]">
      ...
      <div className="space-y-2">
        ...
        <input id="cuenta_caja" name="cuenta_caja" placeholder="Cuenta o subcuenta" className="h-10 w-full rounded-md border border-[#E5E7EB] bg-white px-3 text-sm text-[#111827] outline-none transition placeholder:text-[#9CA3AF] focus:border-[#8A1538] focus:ring-2 focus:ring-[#E9B8C6]" />
      </div>
      <div className="space-y-2">
        <label htmlFor="concepto_caja" className="text-sm font-medium text-[#111827]">Concepto Caja</label>
        <input id="concepto_caja" name="concepto_caja" defaultValue="Pago de comisión" className="h-10 w-full rounded-md border border-[#E5E7EB] bg-white px-3 text-sm text-[#111827] outline-none transition placeholder:text-[#9CA3AF] focus:border-[#8A1538] focus:ring-2 focus:ring-[#E9B8C6]" />
      </div>
      <p className="text-xs leading-5 text-[#6B7280]">
        Al marcar como pagada, se generará un egreso automático en Caja por el neto a cobrar.
      </p>
      {state.error ? (
        <div className="rounded-md border border-[#FEE2E2] bg-[#FEF2F2] px-3 py-2 text-sm text-[#991B1B]">
          {state.error}
        </div>
      ) : null}
...
export function LiquidacionDetail({ liquidacion }: { liquidacion: Liquidacion }) {
  ...
  const vendorLabel = liquidacion.vendedor?.nombre ?? "Sin vendedor";
      ...
      <article className="rounded-md border border-[#E5E7EB] bg-white p-5">
        ...
        <div className="grid gap-4 py-5 md:grid-cols-2 xl:grid-cols-3">
          <div className="space-y-1">
            ...
            <p className="text-sm text-[#6B7280]">Vendedor asignado</p>
          </div>
          <div className="space-y-1">
