...
-- Ejecutar este archivo completo como una única sentencia en Supabase.
-- No depende de ejecutar bloques separados del SQL original.
DO $$
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM public.empleados WHERE activo IS DISTINCT FROM FALSE AND lower(nombre) LIKE '%robinson%') THEN
  RAISE EXCEPTION 'No se encontró un empleado activo Robinson para cargar las comisiones.';
END IF;
...
-- Validación rápida (ejecutar por separado si querés confirmar):
